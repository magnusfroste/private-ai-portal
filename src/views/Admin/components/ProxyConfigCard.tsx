...
import { Server, RefreshCw, CheckCircle2, XCircle, AlertTriangle, ExternalLink, Plus } from "lucide-react";
...
export const ProxyConfigCard = () => {
  ...
  const checkKeyExists = async () => {
    setChecking(true);
    try {
      // Call an edge function that checks if LITELLM_MASTER_KEY is set
      const { data, error } = await supabase.functions.invoke("check-proxy-status");
      if (error) throw error;
      const result = data as ProxyStatus & { has_key: boolean };
    ...
    <Card className="glass-card">
      ...
      <CardContent className="space-y-4">
        {hasKey === null && !checking ? (
          <Button onClick={checkKeyExists} disabled={checking} variant="outline" className="w-full md:w-auto">
          ...
          <div className="space-y-4">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <p className="text-muted-foreground">
                      Portalen behöver en master key för att kommunicera med din LiteLLM-proxy. Utan den kan vi inte skapa API-nycklar, hämta modeller eller visa användarstatistik.
                    </p>
            ...
            <div className="flex flex-wrap gap-2">
              <Button onClick={checkKeyExists} disabled={checking} variant="outline" size="sm">
                <RefreshCw className={`w-3.5 h-3.5 mr-1.5 ${checking ? "animate-spin" : ""}`} />
                Uppdatera
              </Button>
            </div>

            {/* Future proxy support teaser */}
